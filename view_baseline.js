...
function open_advanced_dialog(file_path) {
    ...
    $('#adv_inheritance_filepath').text(file_path);
...
// Update inheritance display 
//TODO: this doesnt work i think. 
function update_inheritance_user() {
    let container = $('#adv_inheritance_div')
    container.empty()

    let username = $('#adv_inheritance_current_user').attr('selected_user')
    ...
    while (current) {
        let perms = get_grouped_permissions(current, username)

        for (let ace_type in perms) {
            for (let perm in perms[ace_type]) {
                if (perms[ace_type][perm].inherited) {
                    results.push({
    ...
    // Display results
    if (results.length === 0) {
        container.append("<p>No inherited permissions.</p>")
        return
    }
...
let header = $('<tr><th>Allowed</th><th>Permissions</th></tr>')
$('#adv_effective_effective_list').append(header)
...
for (let p of Object.values(permissions)) {
    ...
    <tr id="adv_effective_row_${p}">
        ...
        <td style="text-align: center;" id="adv_effective_name_${p}">${p}</td>
    </tr>
    `)
...
$("#adv_inheritance_user_select").click(function (event) {
    open_user_select("adv_inheritance_current_user")
})
...
// listen for mutations for inheritance user permissions tab
inheritance_user_observer = new MutationObserver(function (mutationsList) {
    for (let mutation of mutationsList) {
        if (mutation.type === 'attributes' && mutation.attributeName === 'selected_user') {
            update_inheritance_user()
        }
    }
})

inheritance_user_observer.observe(
    document.getElementById('adv_inheritance_current_user'),
    { attributes: true }
)   
